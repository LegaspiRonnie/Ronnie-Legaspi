...
import { posts, profile, formatDate, limit } from '../content.ts';
...
export default function InsightShow() {
  ...
  return (
    <>
      <article className="py-16 bg-white dark:bg-blue-900 transition-colors duration-300">
        <div className="max-w-3xl mx-auto px-6 lg:px-8">
          ...
          {post.cover_image_url && (
            <Reveal effect="scale" delay={0.1}>
              <img src={post.cover_image_url} alt={post.title} className="w-full rounded-2xl mb-10" />
            </Reveal>
          )}

          <Reveal effect="fade" delay={0.15} className="text-gray-700 dark:text-gray-300 leading-relaxed space-y-5 mb-12">
            {post.body.split('\n\n').map((paragraph, i) => (
              <p key={i}>{paragraph}</p>
            ))}
          </Reveal>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 py-6 border-y border-gray-200 dark:border-gray-800 mb-14">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 px-3.5 py-2 rounded-lg border border-gray-200 dark:border-gray-800 text-sm text-gray-600 dark:text-gray-300 hover:bg-white dark:hover:bg-blue-900 hover:scale-[1.03] active:scale-[0.97] transition-all duration-300"
            >
          ...
          {related.length > 0 && (
            <div>
              ...
              <Stagger stagger={0.1} className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                {related.map((item) => (
                  <StaggerItem key={item.id}>
                    <Link
                      to={`/insights/${item.slug}`}
                      className="group block bg-white dark:bg-blue-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-5 transition-all duration-500 hover:-translate-y-1.5 hover:shadow-lg hover:shadow-gray-200/60 dark:hover:shadow-none hover:border-blue-200 dark:hover:border-blue-900"
                    >
