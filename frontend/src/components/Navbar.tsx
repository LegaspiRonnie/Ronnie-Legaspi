...
export default function Navbar() {
  ...
  const mobileItemClass = (item: (typeof navItems)[number]) =>
    `block px-3 py-2.5 rounded-lg hover:bg-blue-50 dark:hover:bg-blue-950 hover:text-blue-700 dark:hover:text-blue-400 transition-colors duration-300 ${
      isActive(item) ? 'text-blue-700 dark:text-blue-400 bg-blue-50 dark:bg-blue-950' : ''
    }`;
  ...
  const bookACallButton = (extra = '') => (
    <motion.button
      ...
      className={`inline-flex items-center gap-2 px-3.5 py-2 rounded-lg border border-gray-200 dark:border-gray-800 text-gray-700 dark:text-gray-200 text-sm font-medium transition-colors duration-300 hover:bg-white dark:hover:bg-blue-950 hover:border-gray-300 dark:hover:border-gray-700 ${extra}`}
    >
      {CalendarIcon}
      Book a call
    </motion.button>
  ...
  return (
    <header
      className={`fixed top-0 left-0 w-full z-50 bg-white/75 dark:bg-blue-950/75 backdrop-blur-md transition-all duration-300 border-b ${
        scrolled ? 'border-gray-200 dark:border-gray-800 shadow-sm shadow-gray-900/5 bg-white/90 dark:bg-blue-950/90' : 'border-transparent'
      }`}
    >
